...
function drawGraph() {
	...
	// Create categories for legend and colors
	graph.categories = {};
	for (var name in graph.data) {
		var obj = graph.data[name],
	...
	graph.categoryKeys.sort();

	graph.strokeColor = getColorScale(0.7);
	graph.fillColor = getColorScale(-0.01);
	...
	graph.legend = graph.svg.append('g')
		...
		.attr('y', 0)
		.selectAll('.category')
		.data(d3.values(graph.categoryKeys))
		.enter().append('g')
		.attr('class', 'category');
	...
	graph.legend.append('rect')
		...
		.attr('fill', function(d) {
			return graph.fillColor(d);
		})
		.attr('stroke', function(d) {
			return graph.strokeColor(d);
		});
	...
	setTimeout(function() {
		...
		graph.force.start();
		for (var i = 0; i < graph.ticksWithoutCollisions; i++) {
			graph.force.tick();
		}
...
function wrap(text) {
	...
	if (text.length <= graph.maxLineChars) {
		return [text];
	} else {
		for (var k = 0; k < graph.wrapChars.length; k++) {
			var c = graph.wrapChars[k];
			for (var i = graph.maxLineChars; i >= 0; i--) {
				if (text.charAt(i) === c) {
		...
		return [text.substring(0, graph.maxLineChars)]
			.concat(wrap(text.substring(graph.maxLineChars)));
	}
}
...
function tick(e) {
	...
	graph.line
		...
		.each(function(d) {
			...
			var x1 = d.source.x,
                ...
                x2 = d.target.x,
                y2 = d.target.y,
                line = new geo.LineSegment(x1, y1, x2, y2);

			for (var e in d.target.edge) {
				var ix = line.intersect(d.target.edge[e].offset(x2, y2));
				if (ix.in1 && ix.in2) {
					x2 = ix.x;
					y2 = ix.y;
					break;
			...
			for (var e in d.source.edge) {
            	var ix = line.intersect(d.source.edge[e].offset(x1, y1));
            	if (ix.in1 && ix.in2) {
            		x1 = ix.x;
            		y1 = ix.y;
            		break;
	...
	// Bundle to groups | START
	var coords = {};
	var groups = [];

	// sort the nodes into groups:
	graph.node.each(function(d) {
		if (groups.indexOf(d.type) == -1) {
			groups.push(d.type);
			coords[d.type] = [];
		}
		coords[d.type].push({
	...
	graph.node.each(function(d) {
		var cx = centroids[d.type].x;
		var cy = centroids[d.type].y;
		var x = d.x;
		var y = d.y;
		var dx = cx - x;
		var dy = cy - y;
