...
class ExcelReader @Autowired constructor(val serverProperties: ServerProperties) {
    ...
    private fun parseLinks(
        ...
        for (currentRow in 2..sheet.lastRowNum) {
            ...
            val sourceID =
                row.getCell(ExcelColumnNumberLinks.FIRST_APP_ID.value).stringCellValue.trim()
            val targetID =
                row.getCell(ExcelColumnNumberLinks.SECOND_APP_ID.value).stringCellValue.trim()
            val protocol =
                row.getCell(ExcelColumnNumberLinks.PROTOCOL.value).stringCellValue.trim()
            val interfaceName =
                row.getCell(ExcelColumnNumberLinks.INTERFACE_NAME.value).stringCellValue.trim()
    ...
    private fun parseNodes(
        ...
        for (currentRow in 1..sheet.lastRowNum) {
            ...
            val row = sheet.getRow(currentRow)
            val id =
                row.getCell(ExcelColumnNumberApplications.ID.value).stringCellValue.trim()
            val name =
                row.getCell(ExcelColumnNumberApplications.NAME.value).stringCellValue.trim()
            val description =
                row.getCell(ExcelColumnNumberApplications.DESCRIPTION.value).stringCellValue.trim()
            val cluster =
                row.getCell(ExcelColumnNumberApplications.SUB_CLUSTER.value).stringCellValue.trim()
            val status =
                row.getCell(ExcelColumnNumberApplications.STATUS.value).stringCellValue.trim()
            val location =
                row.getCell(ExcelColumnNumberApplications.LOCATION.value).stringCellValue.trim()
